...
namespace fkkt
{
    namespace editpage
    {
        ...
        BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
        {
            ...
            Window window =
                CreateWindowW(szWindowClass, appTitleString, WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, 0, CW_USEDEFAULT, 0, nullptr, nullptr, hInstance, nullptr);
            if (!window)
            {
                dialog->showMsg("Failed to create window!", "Error");
                return false;
            }
        ...
        LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
        {
            ...
            case WM_CREATE:
            {
                ...
                editBox                = CreateWindowEx(
                    ...
                    screenHeight, /* Height */
                    hWnd,         /* Parent Window */
                    (HMENU)ID_MAIN_EDIT_BOX,
                    hInstance,
                    nullptr);
                if (!editBox)
                {
                    dialog->showMsg("Failed to create edit control!", "Error");
                    return false;
                }
            ...
            case WM_COMMAND:
            {
                ...
                switch (wmId)
                {
                case IDM_NEW: SendMessage(editBox, WM_SETTEXT, 0, reinterpret_cast<LPARAM>(L"")); break;
                case IDM_SAVE: dialog->showMsg("已保存"); break;
